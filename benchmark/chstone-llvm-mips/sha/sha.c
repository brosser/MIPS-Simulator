...
void
mem_set (LONG * s, int c, int n, int e)
{
  unsigned long uc;
  unsigned long *p;
  int m;
...
void
mem_cpy (LONG * s1, const BYTE * s2, int n) // hycheah -- changed name memcpy to mem_cpy to remove compiler warnings
{
  unsigned long *p1;
  unsigned char *p2;
  unsigned long tmp;
...
sha_update (const BYTE * buffer, int count)
{
  ...
  while (count >= SHA_BLOCKSIZE)
    {
      mem_cpy (sha_info_data, buffer, SHA_BLOCKSIZE);
      sha_transform ();
      buffer += SHA_BLOCKSIZE;
      count -= SHA_BLOCKSIZE;
    }
  mem_cpy (sha_info_data, buffer, count);
}
...
sha_final ()
{
  ...
  if (count > 56)
    {
      mem_set (sha_info_data, 0, 64 - count, count);
      sha_transform ();
      mem_set (sha_info_data, 0, 56, 0);
    }
  else
    {
      mem_set (sha_info_data, 0, 56 - count, count);
    }
